...
import com.justai.gramlin.scenario.utils.asStatePath
        ...
        onUpdateUserStatus {
            ...
            if (contactId == null || contactId == userId) {
                stats.getOrPut(userId) {
                    reactions.api.send(TdApi.GetUser(userId)).let(CollectContactUsageStats::Stats)
            ...
            state("all") {
                action {
                    ...
                    if (users.isEmpty()) {
                        ...
                        reactions.sendPhoto(chartImageUrl, "Here is an overall usage statistics starting from ${startTime.format(
                            dateTimeFormatter
                        )}")
