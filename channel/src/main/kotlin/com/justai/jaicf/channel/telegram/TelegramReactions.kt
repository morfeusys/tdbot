...
class TelegramReactions(
    ...
    private fun addResponse(pair: Pair<retrofit2.Response<Response<Message>?>?, Exception?>) {
        pair.first?.body()?.result?.let(::addResponse)
    }

    private fun addResponse(result: TelegramBotResult<Message>) =
        addResponse(result.get())
    ...
    ) = sendMessage(text, parseMode, disableWebPagePreview, disableNotification, replyToMessageId, allowSendingWithoutReply, replyMarkup)
    ...
    fun image(
        ...
        allowSendingWithoutReply: Boolean? = null,
        replyMarkup: ReplyMarkup? = null
    ) = sendPhoto(url, caption, parseMode, disableNotification, replyToMessageId, allowSendingWithoutReply, replyMarkup)
    ...
    fun sendPhoto(
        ...
        api.sendPhoto(
            chatId, url, caption, parseMode, disableNotification, replyToMessageId, allowSendingWithoutReply, replyMarkup
        ).also { addResponse(it) }
    ...
    fun sendVideo(
        ...
        chatId, url, duration, width, height, caption, disableNotification, replyToMessageId, allowSendingWithoutReply, replyMarkup
    ).also { addResponse(it) }
    ...
    fun sendAudio(
        ...
        api.sendAudio(
            chatId, url, duration, performer, title, disableNotification, replyToMessageId, allowSendingWithoutReply, replyMarkup
        ).also { addResponse(it) }
    ...
    fun sendDocument(
        ...
        chatId, url, caption, parseMode, disableContentTypeDetection, disableNotification, replyToMessageId, allowSendingWithoutReply, replyMarkup
    ).also { addResponse(it) }
    ...
    fun sendLocation(
        ...
        chatId, latitude, longitude, livePeriod, disableNotification, replyToMessageId, allowSendingWithoutReply, replyMarkup
    ).also { addResponse(it) }
    ...
    fun sendInvoice(
        ...
        chatId, paymentInvoiceInfo, disableNotification, replyToMessageId, allowSendingWithoutReply, inlineKeyboardMarkup
    ).also { addResponse(it) }
